---
name: ozm-security-review
description: Use for OZM-managed security, credential, permission, authz/authn, secret, network boundary, or payload-less risk work.
---
...
| Field | Contract |
| --- | --- |
| Applicability | OZM-governed security, credential, permission, authz/authn, secret, network boundary, or payload-less risk work. |
...
| Output receipt | `security_proof_matrix`, `external_prerequisite_table`, and `rollback_recovery_plan`. |
...
activation_effect_contract:
  owner_question:
    - "What domain proof, prerequisite, negative test, and rollback/recovery evidence is required before this security review claim can rise?"
  blocks_when:
  ...
  non_surface_failure_code:
    - ozm-security-review_loaded_without_required_activation_effect
```
