---
name: ozm-database-migration
description: Use for OZM-managed schema migration, data migration, rollback, seed, index, storage table, enum, or persistence change work.
---
...
| Field | Contract |
| --- | --- |
| Applicability | OZM-governed schema migration, data migration, rollback, seed, index, storage table, enum, or persistence change work. |
...
| Output receipt | `migration_proof_matrix`, `external_prerequisite_table`, and `rollback_recovery_plan`. |
...
activation_effect_contract:
  owner_question:
    - "What domain proof, prerequisite, negative test, and rollback/recovery evidence is required before this database migration claim can rise?"
  blocks_when:
  ...
  non_surface_failure_code:
    - ozm-database-migration_loaded_without_required_activation_effect
```
